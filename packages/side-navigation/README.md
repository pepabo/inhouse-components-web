...
最上位の遷移先を示す [Interactive List](../interactive-list/README.md) を配置します。タップ／クリックで該当の画面へ遷移します。
...
- **モバイルなどの小さな画面で遷移先が5つを超える場合**: 優先順位をつけて上位5つを Bottom Navigation に、それ以外を [Navigation Drawer](../navigation-drawer/README.md) に配置します。

#### 類似コンポーネントとの違い

- **[Navigation Drawer](../navigation-drawer/README.md)**: モバイルなどの小さな画面で、Bottom Navigation に収まらない優先順位の低い遷移先を格納するドロワー型のナビゲーション。常に表示される Side Navigation とは表示形態が異なります。
