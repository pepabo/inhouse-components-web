...
各コンポーネントパッケージには、デザインドキュメント（使い分け・バリエーション・ステート・アクセシビリティ・ライティングの規約）が README.md の「デザインドキュメント」節として同梱されています。あわせて `CLAUDE.md` / `AGENTS.md` も同梱しており、Claude Code などの AI エージェントがパッケージ内のファイル（mixin の SCSS など）を読んだ際に、自動でデザインドキュメントへ誘導されます。
